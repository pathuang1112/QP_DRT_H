...
def fidelity(bit_a, bit_b):
    ...
    return fid
class DRT_F():
    def __init__(self, number_of_variables, input_qbit_count, output_qbit_count, group_name, program_name, algorithm, iter_num):
        #self.title = "ART_1"
      ...
      #  super().__init__()
        ...
        oracle_path = os.path.join("./oracle/OpenQASM/" + self.program_name + "_oracle.xlsx")
        oracle_data = pd.read_excel(oracle_path, sheet_name=self.group_name, dtype=str)     #input	bin_input	result	p_value
        self.All_candidates = []
        self.oracle = []
    ...
    def calculate_quantum_distance(self, a, b):
        ...
        D = 1 - F
        return D
    def custom_hamming_k_medoids(self, data, k):
        ...
        for _ in range(10):
            ...
            for i in range(k):
                if clusters[i]:
                    ...
                    # cluster_int = [[int(bit) for bit in binary_string] for binary_string in clusters[i]] # 将二进制字符串转换为整数数组
    ...
    def run(self):
        ...
        while iter < self.iter_num:
            ...
            cluster_labels, centroids = self.custom_hamming_k_medoids(self.All_candidates, k)
            ...
            for n_total in range(self.upper_bound):
                ...
                test_input = testsuit[cluster_name][test_index][1]  # input
                test_label = testsuit[cluster_name][test_index][0]  # result
                test_pvalue = testsuit[cluster_name][test_index][3] # pvalue
                ...
                if test_label == 'False':
                    ...
                    if testsuit["probability_profile"][Cl] < delta:
                        for i in range(n_profile):
                            if i != Cl:
                                delta_i = testsuit["probability_profile"][Cl] * Dmatrix[i][Cl] / sum(Dmatrix[Cl])
