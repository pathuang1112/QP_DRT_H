...
if __name__ == '__main__':
    ...
    set_mode = int(sys.argv[6]) 
    oracle_path = os.path.join("./oracle/OpenQASM/" + group_name + "_RQ3oracle.xlsx")
    cand_num = int(sys.argv[6]) # for ART, number of the candicates

    result_path = './result/RQ3/' + group_name
    ...
    folder = './result/RQ3/' + group_name
    if not os.path.exists(folder):
        os.makedirs(folder)

    num_vars = -1  #number of the testcases to be selected
    if input_qbit_count == 10:
        num_vars = 614 # 2**10*3*0.1=614.4
    elif input_qbit_count == 11:
        num_vars = 1229  # 2**11*3*0.1=1228.8
    ...
    if alg == "RT":
        ...
        algorithm = DRT_H(num_vars, input_qbit_count, output_qbit_count, group_name, program_name, alg, iter_num, oracle_path, "mode" + str(set_mode))
    else:
    ...
    count_fail = {}
    if alg == 'ART_1' :
        file_path = './result/RQ3/' + group_name + '/' + program_name + '_' + alg + '(cand_num=' + str(cand_num) + ').txt'
    elif alg == 'RT' or alg == 'DRT_F' or alg == 'DRT_1' or alg == 'RPT_1':
        file_path = './result/RQ3/' + group_name + '/' + program_name + '_' + alg + '.txt'
    elif alg == 'DRT_H':
        file_path = './result/RQ3/' + group_name + '/' + program_name + '_' + alg + '_mode' + str(set_mode) + '.txt'
    else:
        pass
    ...
    with open('./result/RQ3/' + group_name + '/' + program_name + '_' + alg + '_mode' + str(set_mode) + '_failCount.txt', "w") as f2:
        f2.write("count_fail:\n")
    ...
    if alg == 'ART_1':
        os.rename('./result/RQ3/' + group_name + '/' + program_name + '_' + alg + '_failCount.txt',
                  './result/RQ3/' + group_name + '/' + program_name + '_' + alg + '_failCount(cand_num=' + str(cand_num) + ').txt')
    ...
    try:
        workbook1 = openpyxl.load_workbook('./result/RQ3/' + avg_filename)
    except FileNotFoundError:
        workbook1 = openpyxl.Workbook()
    sheet_name = group_name
    if sheet_name not in workbook1.sheetnames:
        worksheet = workbook1.create_sheet(sheet_name) 
        if alg == 'ART_1' or alg == 'RPT_1' or alg == 'DRT_1' or alg == 'DRT_F' or alg == 'DRT_H':
            worksheet.cell(row=1, column=2, value='mode' + str(set_mode))  
        for index, key in enumerate(avg_failCount.keys()):  
            worksheet.cell(row=index + 2, column=1, value=key)
        empty_column = 2
    else:
        worksheet = workbook1[sheet_name]
        empty_column = worksheet.max_column + 1
        if alg == 'ART_1' or alg == 'RPT_1' or alg == 'DRT_1' or alg == 'DRT_F' or alg == 'DRT_H':
            worksheet.cell(row=1, column=empty_column, value='mode' + str(set_mode))

    col_idx = empty_column
    for index, key in enumerate(avg_failCount.keys()):
        worksheet.cell(row=index + 2, column=col_idx, value=avg_failCount[key])

    workbook1.save('./result/RQ3/' + avg_filename)


    try:
        workbook2 = openpyxl.load_workbook('./result/RQ3/' + max_filename)
    except FileNotFoundError:
        workbook2 = openpyxl.Workbook()

    sheet_name = group_name
    if sheet_name not in workbook2.sheetnames:
        worksheet = workbook2.create_sheet(sheet_name)
        if alg == 'ART_1' or alg == 'RPT_1' or alg == 'DRT_1' or alg == 'DRT_F' or alg == 'DRT_H':
            worksheet.cell(row=1, column=2, value='_mode' + str(set_mode))
        for index, key in enumerate(max_failCount.keys()):
            worksheet.cell(row=index + 2, column=1, value=key)
        empty_column = 2
    else:
        worksheet = workbook2[sheet_name]
        empty_column = worksheet.max_column + 1
        if alg == 'ART_1' or alg == 'RPT_1' or alg == 'DRT_1' or alg == 'DRT_F' or alg == 'DRT_H':
            worksheet.cell(row=1, column=empty_column, value='_mode' + str(set_mode))

    col_idx = empty_column
    for index, key in enumerate(max_failCount.keys()):  #使用`enumerate` 函数来获取数据列表中每个元素的索引
        worksheet.cell(row=index + 2, column=col_idx, value=max_failCount[key])
    workbook2.save('./result/RQ3/' + max_filename)
